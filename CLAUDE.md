...
> 📌 **[docs/handoff-2026-08-27.md](docs/handoff-2026-08-27.md)를 같이 읽는다.**
> 이 문서가 *코드가 어떻게 생겼는지*를 적는다면, 그쪽은 **지금 어디까지 왔는지,
> 다음에 뭘 할지, 그리고 재는 방법에서 밟은 함정**을 적는다.
> 마지막 것이 특히 중요하다 — 같은 도구를 쓰면 또 밟고, 전부 결과가 좋아 보이는
> 쪽으로 틀린다.
>
> **사용자가 정한 규칙(하지 말 것, 손대지 말 것, 말투, 커밋 메시지)은
> [docs/handoff-2026-08-21.md](docs/handoff-2026-08-21.md) §1이 그대로 유효하다.**
...
npm run lint             # oxlint
npm run check:css        # styles.X 를 부르는데 정의가 없는 곳 (아래 참고)
npx tsc -b --noEmit      # 타입만 빠르게 확인
npx vite preview --port 4188   # 배포본 그대로 확인. server.proxy를 물려받는다
```

**테스트 프레임워크가 없다.** 테스트 러너도 테스트 파일도 없으므로, 변경 검증은
위 네 개 + 브라우저 확인으로 한다. 테스트를 새로 도입하려면 먼저 사용자에게 묻는다.

`check:css`는 **한 방향만** 본다 — `styles.X`를 부르는데 `.module.css`에 그 클래스가
없는 경우다. 그때 `undefined`가 조용히 들어가 `class` 속성이 통째로 빠지는데
lint·tsc·build가 전부 통과한다. 실제로 `e7baca9`에서 CSS만 지우고 TSX를 남겨
위험도 점 표시가 사라졌고, 화면을 캡처해 비교하고 나서야 알았다.
반대 방향(안 쓰는 CSS)은 일부러 넣지 않았다 — 오탐이 섞인 검사는 곧 무시된다.
**커밋 훅에는 아직 안 걸었다**(사용자 확인 대기).
...
### 데이터도 미리 부른다 — `src/api/warmup.ts` (2026-08-26)

위 둘은 **파일**을 당기고 이건 **데이터**를 당긴다. 요청이 `useEffect` 안에 있으면
"화면이 한 번 그려진 다음"이라는 순서가 강제되는데, 어느 화면인지는 주소만 봐도 안다.
...
부르는 곳이 둘이다 — `main.tsx`가 `createRoot` **앞에서** 주소를 보고 한 번,
표·카드·검색 결과의 링크가 **눌리는 순간**(`pointerdown`) 한 번.

⚠️ **캐시가 아니다.** 한 번 가져가면 지운다. 시세는 초 단위로 바뀌는 값이라 들고
있다가 나중에 내주면 옛날 값을 새 값인 척 보여주게 된다.

⚠️ **마우스를 올릴 때(hover) 부르면 안 된다.** 표를 훑기만 해도 수십 건이 나가고,
그게 홈이 102종목을 몰아쳐 계정 경고를 받은 그 자리다. 누르기는 실제로 옮겨 가는
1회당 1건이다.

⚠️ **`<head>`에서 fetch를 먼저 던지는 방법은 일부러 안 골랐다.** 그쪽이 두 배 넘게
빠르지만(최대 1.2초) fetch 창구가 `client.ts` 말고 하나 더 생기고, 브라우저가 그것을
앱의 요청과 같은 것으로 쳐 주지 않으면 조용히 두 번 부른다. 이 창구는 백엔드에서
**KIS를 1건 부른다.**

기간 값 `DETAIL_PERIOD`가 화면이 아니라 `api/stock.ts`에 있는 이유도 이것이다.
미리 부르는 쪽과 화면이 다른 기간으로 부르면 미리 부른 것이 버려지고 두 번 나간다.
...
`getQuotes()`가 `QuoteResult.failure`로 처음 걸린 실패 하나를 들고 오는 것도 같은
이유다 — 예전에는 종목별 실패를 `console.warn`으로 흘리고 끝내서, 표는 "-"로
채워지지만 **화면이 왜 비었는지 말할 근거가 없었다.**

⚠️ **`toUserMessage`는 화면 전체가 죽었을 때 쓰라고 만든 문구다.** '아직 준비되지
않은 기능입니다'처럼 판정을 내린다. 홈처럼 **카드도 표도 떠 있고 값만 비어 있는**
자리에 그대로 쓰면 "준비 안 됐다는데 카드는 있네?"가 된다. 그래서 `HomePage`는
`toReason()`을 따로 두고 판정 대신 **원인**을 적는다(가르는 기준은 `error.ts`와 같다).

**흰 화면 방지도 두 겹이다.** `App.tsx`의 `ErrorBoundary`가 `<main>` 안쪽 전체를
감싸고, `StockChartPage`는 **뉴스만** 따로 한 번 더 감싼다. 뉴스가 그리다 터져도
차트·시세까지 사라지면 안 되기 때문이다. 그때 쓰라고 `ErrorBoundary`에 `fallback`
프로퍼티가 있다 — 화면 한 칸만 감싼 경계는 앱 전체가 멈춘 것처럼 말하면 안 된다.
...
> ⚠️ **`vite.config.ts`의 주석 한 줄이 사실과 다르다** (2026-08-27 확인).
> 거기 "그런 파일은 없으므로 **404가 된다**"고 적혀 있는데, 실제로는 **200에
> `index.html`**이 온다 — vite의 SPA 폴백이 먼저 걸린다.
> **막는다는 것 자체는 사실이다**(개발 서버 로그의 `[차단]`으로 확인했고, 백엔드까지
> 한 글자도 안 나간다). JSON 대신 HTML이 오므로 `client.ts`가 실패로 처리하는 결과도
> 같다. 응답 모양만 다르다. 사용자에게 알렸고 **고치지 않기로 두었다.**
...
### 지금 화면이 어떻게 보이는지 (2026-08-27 배포본 확인)

| 화면 | 상태 |
| --- | --- |
| 투자성향테스트 | **10문항이 실제로 뜬다.** 백엔드 DB에 문항이 들어왔다 |
| 백테스트 | 전략 목록이 실제로 뜬다 |
| **종목 상세** | 차트·시세는 실제로 뜬다. **뉴스는 2026-08-25부터 백엔드가 502다** |
| 홈 | 종목명·코드는 뜨고 시세는 `-`. **고장이 아니라 KIS 차단 때문이고, 이제 화면이 그 사실을 말한다** |
| AI 주가분석 | 허용 목록에 없어 못 부른다. 빈 화면에 예시 질문 3개 |
| 사용설명서 | `준비중입니다` 한 줄. `api/guide`가 mock이라 **백엔드 없이 채울 수 있다** |
| 마이페이지 | 토큰이 있으면 요청은 나간다 |

**9개 화면 콘솔 오류 0건.** 레이아웃 밀림은 홈 0.040 · AI 0.045 · 백테스트 0.086 ·
성향테스트 0.099 · 상세 0.104(**상세만 기준선 0.1 위**).
...
- 스타일은 CSS Modules(`*.module.css`). 색상은 `src/index.css`의 CSS 변수를 쓴다.
  ...
  이 규율이 잘 지켜져 있다 — `var(--color-…)` 347곳에 직접 박은 16진수는 4곳뿐이다
  (나머지 23곳은 `index.css`의 팔레트 정의 자체). 다크 모드를 붙인다면 그 덕에
  `@media (prefers-color-scheme: dark)` 블록 하나로 거의 끝난다.
- 글꼴은 **Pretendard 가변(variable)** 이다. 굵기별 파일이 따로인 static을 쓰면
  화면이 쓰는 네 굵기(400·500·600·700)가 **같은 한글 구간을 네 번씩** 받는다
  (실측 29건 344KB → 가변 11건 266KB). `index.html`·`index.css`·`CandleChart.tsx`
  세 곳의 이름이 같아야 한다 — 하나만 어긋나면 거기만 다른 글꼴이 된다.
