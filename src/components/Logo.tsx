 ...
 *
 * 글자가 폰트가 아니라 도형이다. 예전에는 구글 폰트(Michroma)를 받느라 외부 서버에
 * 세 번 다녀왔고, 늦게 오면 로고가 잠깐 다른 글씨로 보였다가 바뀌었다.
 *
 * 파비콘(public/favicon.svg)은 문서 밖이라 이 파일을 못 쓴다. 같은 도형을 복사해
 * 뒀으니 심볼을 고치면 그쪽도 함께 고친다.
 */
...
/**
 * 심볼 — 싹 튼 캔들. 상승 캔들의 위쪽 심지가 올라가면서 색이 빨강에서 초록으로
 * 넘어가 그대로 줄기가 된다. 이음매가 없는 것이 이 도형의 요점이다.
 * 색을 CSS 변수로 빼지 않은 건 테마를 따라 바뀌면 안 되고 파비콘과도 맞아야 해서다.
 */
...
/**
 * 글자 — JUBY. 선이 아니라 면으로 그려서 획 안에서 굵기가 변한다 — U와 J의 바닥
 * 곡선이 세로획보다 가늘고(곡선은 눈에 더 두꺼워 보인다), 둥근 글자는 기준선(y=100)을
 * 103까지 넘어간다. Y만 왼쪽으로 12 당겼다 — 팔이 위에서 벌어져 있어 같은 간격을
 * 주면 아래에 빈 쐐기가 생긴다. 색은 currentColor다.
 */
