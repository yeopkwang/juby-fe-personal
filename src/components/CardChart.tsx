 ...
 *
 * 예전에는 recharts로 그렸다. 그런데 이 앱에는 차트 라이브러리가 이미 하나 더 있다 —
 * 종목 상세의 캔들차트가 쓰는 lightweight-charts다. 같은 앱에서 두 벌을 받는 셈이라
 * **이 카드 세 장 때문에 gzip 100KB가 더 들어왔다.**
 *
 * 그렇다고 상세 쪽 라이브러리로 합칠 수도 없었다. 그건 축·눈금·마우스 조작이 딸린
 * 본격적인 차트라, 눈금도 축도 없는 이 작은 그림에는 오히려 더 무겁고 다루기 번거롭다.
 * 필요한 게 막대 몇 개와 선 두 개뿐이라 직접 그리는 쪽이 짧다.
 *
 * 부르는 쪽(TopStockCard)이 여전히 lazy로 가져간다. 이제 가벼워져서 큰 의미는 없지만,
 * 홈이 표를 먼저 그리고 그래프는 일봉이 온 뒤에 붙이는 순서 자체는 그대로 두는 게 맞다.
 *
...
/**
 * 거래량 막대가 차지할 수 있는 최대 높이를 그래프의 1/4로 묶는 값.
 *
 * 막대 눈금의 위쪽 끝을 실제 최대 거래량의 4배로 잡는다는 뜻이다. 그래야 가장 큰 막대도
 * 아래 1/4에만 머문다. 막대는 주가 흐름을 읽는 걸 방해하지 않을 만큼만 보이면 된다.
 */
const VOLUME_HEADROOM = 4

/**
 * 이 요소의 실제 가로 폭. 창 크기가 바뀌면 따라 바뀐다.
 *
 * SVG는 그릴 좌표를 숫자로 받아야 해서 폭을 알아야 한다. viewBox로 늘리는 방법도 있지만
 * 그러면 가로만 늘어나는 만큼 선 굵기와 끝점 동그라미까지 같이 찌그러진다.
 */
